import android.content.Context;
import android.util.Base64;
import android.util.Log;
import java.io.File;
import java.io.IOException;
import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

public class bra
{
  private static final String a = bra.class.getSimpleName();
  private Context b;
  private String c;
  private JSONObject d;
  
  public bra(Context paramContext)
  {
    b = paramContext;
    c = null;
    d = j();
    bsz.b();
    bsz.a(d);
  }
  
  public bra(Context paramContext, String paramString)
  {
    this(paramContext, paramString, (byte)0);
  }
  
  private bra(Context paramContext, String paramString, byte paramByte)
  {
    bsz.b();
    b = paramContext;
    c = paramString;
    paramContext = o();
    if ("".equals(paramContext)) {
      throw new IOException("No valid config found for " + paramString);
    }
    bsz.b();
    try
    {
      paramString = new File(b.getFilesDir(), "CONFIG_DATA");
      File localFile = new File(b.getFilesDir(), "CONFIG_TIME");
      bwy.a(paramString, paramContext);
      bwy.a(localFile, String.valueOf(System.currentTimeMillis()));
      bsz.b();
      d = new JSONObject(paramContext);
      bsz.b();
      return;
    }
    catch (IOException paramString)
    {
      for (;;)
      {
        new StringBuilder("Failed to write config data: ").append(paramString.toString());
      }
    }
  }
  
  private JSONObject a(String paramString)
  {
    try
    {
      bsz.b();
      JSONObject localJSONObject = new JSONObject(bsz.c(b, paramString));
      bsz.b();
      return localJSONObject;
    }
    catch (Exception localException)
    {
      bsz.a("PRD", "Error while loading prdc Config " + paramString, localException);
    }
    return null;
  }
  
  private static JSONObject a(JSONObject paramJSONObject1, JSONObject paramJSONObject2)
  {
    try
    {
      bsz.b();
      Iterator localIterator = paramJSONObject2.keys();
      while (localIterator.hasNext())
      {
        String str = (String)localIterator.next();
        bsz.b();
        paramJSONObject1.put(str, paramJSONObject2.get(str));
      }
      bsz.b();
    }
    catch (Exception paramJSONObject1)
    {
      bsz.a("PRD", "Error encountered while applying prdc Config", paramJSONObject1);
      return null;
    }
    return paramJSONObject1;
  }
  
  private JSONObject j()
  {
    for (;;)
    {
      try
      {
        JSONObject localJSONObject = l();
        if (localJSONObject != null)
        {
          if (bsz.a(localJSONObject.optString("conf_version", ""), "3.0"))
          {
            long l1 = Long.parseLong(r());
            long l2 = localJSONObject.optLong("conf_refresh_time_interval", 0L);
            if (System.currentTimeMillis() <= l1 + l2 * 1000L) {
              break label234;
            }
            i = 1;
            if (i != 0) {
              n();
            }
            bsz.b();
            return localJSONObject;
          }
          q();
        }
        localJSONObject = k();
        if (localJSONObject == null)
        {
          Log.e(a, "default Configuration loading failed,Using hardcoded config");
          return m();
        }
        localObject = bsz.b(b, "prdc");
        if (localObject == null)
        {
          n();
          bsz.a(3, "PRD", "prdc field not configured, using default config");
          return localJSONObject;
        }
      }
      catch (Exception localException)
      {
        bsz.a("PRD", "Severe Error while loading config, using hard code version", localException);
        return m();
      }
      bsz.a(3, "PRD", "prdc field is configured, loading path:" + (String)localObject);
      Object localObject = a((String)localObject);
      if (localObject == null)
      {
        bsz.a(6, "PRD", "prdc Configuration loading failed, using default config");
        return localException;
      }
      localObject = a(localException, (JSONObject)localObject);
      if (localObject == null)
      {
        bsz.a(6, "PRD", "applying prdc Configuration failed, using default config");
        return localException;
      }
      bsz.a(3, "PRD", "prdc configuration loaded successfully");
      return (JSONObject)localObject;
      label234:
      int i = 0;
    }
  }
  
  private static JSONObject k()
  {
    
    try
    {
      Object localObject = new String(Base64.decode("", 0), "UTF-8");
      bsz.b();
      localObject = new JSONObject((String)localObject);
      return (JSONObject)localObject;
    }
    catch (Exception localException)
    {
      bsz.a("PRD", "Read default config file exception.", localException);
      bsz.b();
    }
    return null;
  }
  
  private JSONObject l()
  {
    
    try
    {
      Object localObject = p();
      if (!"".equals(localObject))
      {
        bsz.b();
        localObject = new JSONObject((String)localObject);
        return (JSONObject)localObject;
      }
    }
    catch (Exception localException)
    {
      bsz.a();
      bsz.b();
    }
    return null;
  }
  
  private static JSONObject m()
  {
    bsz.b();
    JSONObject localJSONObject = new JSONObject();
    try
    {
      localJSONObject.put("conf_version", "3.0");
      localJSONObject.put("async_update_time_interval", 3600);
      localJSONObject.put("forced_full_update_time_interval", 1800);
      localJSONObject.put("conf_refresh_time_interval", 86400);
      localJSONObject.put("location_min_accuracy", 500);
      localJSONObject.put("location_max_cache_age", 1800);
      localJSONObject.put("endpoint_url", "https://svcs.paypal.com/AccessControl/LogRiskMetadata");
      bsz.b();
      return localJSONObject;
    }
    catch (JSONException localJSONException)
    {
      for (;;) {}
    }
  }
  
  private static void n()
  {
    bsz.b();
    bre.a().b();
  }
  
  /* Error */
  private String o()
  {
    // Byte code:
    //   0: aconst_null
    //   1: astore_3
    //   2: invokestatic 40	bsz:b	()V
    //   5: new 62	java/lang/StringBuilder
    //   8: dup
    //   9: invokespecial 268	java/lang/StringBuilder:<init>	()V
    //   12: astore 4
    //   14: new 270	java/net/URL
    //   17: dup
    //   18: aload_0
    //   19: getfield 30	bra:c	Ljava/lang/String;
    //   22: invokespecial 271	java/net/URL:<init>	(Ljava/lang/String;)V
    //   25: invokevirtual 275	java/net/URL:openStream	()Ljava/io/InputStream;
    //   28: astore_1
    //   29: new 277	java/io/BufferedReader
    //   32: dup
    //   33: new 279	java/io/InputStreamReader
    //   36: dup
    //   37: aload_1
    //   38: ldc -24
    //   40: invokespecial 282	java/io/InputStreamReader:<init>	(Ljava/io/InputStream;Ljava/lang/String;)V
    //   43: invokespecial 285	java/io/BufferedReader:<init>	(Ljava/io/Reader;)V
    //   46: astore_2
    //   47: aload_2
    //   48: invokevirtual 288	java/io/BufferedReader:readLine	()Ljava/lang/String;
    //   51: astore_3
    //   52: aload_3
    //   53: ifnull +30 -> 83
    //   56: aload 4
    //   58: aload_3
    //   59: invokevirtual 71	java/lang/StringBuilder:append	(Ljava/lang/String;)Ljava/lang/StringBuilder;
    //   62: pop
    //   63: goto -16 -> 47
    //   66: astore 4
    //   68: aload_1
    //   69: astore_3
    //   70: aload 4
    //   72: astore_1
    //   73: aload_3
    //   74: invokestatic 291	bwy:a	(Ljava/io/Closeable;)V
    //   77: aload_2
    //   78: invokestatic 291	bwy:a	(Ljava/io/Closeable;)V
    //   81: aload_1
    //   82: athrow
    //   83: aload_1
    //   84: invokestatic 291	bwy:a	(Ljava/io/Closeable;)V
    //   87: aload_2
    //   88: invokestatic 291	bwy:a	(Ljava/io/Closeable;)V
    //   91: invokestatic 40	bsz:b	()V
    //   94: aload 4
    //   96: invokevirtual 74	java/lang/StringBuilder:toString	()Ljava/lang/String;
    //   99: areturn
    //   100: astore_1
    //   101: aconst_null
    //   102: astore_2
    //   103: goto -30 -> 73
    //   106: astore 4
    //   108: aconst_null
    //   109: astore_2
    //   110: aload_1
    //   111: astore_3
    //   112: aload 4
    //   114: astore_1
    //   115: goto -42 -> 73
    // Local variable table:
    //   start	length	slot	name	signature
    //   0	118	0	this	bra
    //   28	56	1	localObject1	Object
    //   100	11	1	localObject2	Object
    //   114	1	1	localObject3	Object
    //   46	64	2	localBufferedReader	java.io.BufferedReader
    //   1	111	3	localObject4	Object
    //   12	45	4	localStringBuilder	StringBuilder
    //   66	29	4	localObject5	Object
    //   106	7	4	localObject6	Object
    // Exception table:
    //   from	to	target	type
    //   47	52	66	finally
    //   56	63	66	finally
    //   14	29	100	finally
    //   29	47	106	finally
  }
  
  private String p()
  {
    Object localObject = new File(b.getFilesDir(), "CONFIG_DATA");
    try
    {
      localObject = bwy.a((File)localObject);
      return (String)localObject;
    }
    catch (IOException localIOException)
    {
      bsz.a();
    }
    return "";
  }
  
  private boolean q()
  {
    try
    {
      bsz.b();
      File localFile1 = new File(b.getFilesDir(), "CONFIG_DATA");
      File localFile2 = new File(b.getFilesDir(), "CONFIG_TIME");
      if (localFile1.exists())
      {
        bsz.b();
        localFile1.delete();
      }
      if (localFile2.exists())
      {
        bsz.b();
        localFile1.delete();
      }
      bsz.b();
      return true;
    }
    catch (Exception localException)
    {
      bsz.a();
    }
    return false;
  }
  
  private String r()
  {
    Object localObject = new File(b.getFilesDir(), "CONFIG_TIME");
    try
    {
      localObject = bwy.a((File)localObject);
      return (String)localObject;
    }
    catch (IOException localIOException) {}
    return "";
  }
  
  public final String a()
  {
    return c;
  }
  
  public final String b()
  {
    return d.optString("conf_version", "");
  }
  
  public final long c()
  {
    return d.optLong("async_update_time_interval", 0L);
  }
  
  public final long d()
  {
    return d.optLong("forced_full_update_time_interval", 0L);
  }
  
  public final long e()
  {
    return d.optLong("comp_timeout", 0L);
  }
  
  public final List f()
  {
    ArrayList localArrayList = new ArrayList();
    JSONArray localJSONArray = d.optJSONArray("android_apps_to_check");
    int i = 0;
    while ((localJSONArray != null) && (i < localJSONArray.length()))
    {
      localArrayList.add(localJSONArray.getString(i));
      i += 1;
    }
    return localArrayList;
  }
  
  public final String g()
  {
    return d.optString("endpoint_url", null);
  }
  
  public final boolean h()
  {
    return d.optBoolean("endpoint_is_stage", false);
  }
  
  public final bsx i()
  {
    try
    {
      Object localObject = d.optString("CDS", "");
      if ((localObject == null) || ("".equals(localObject)))
      {
        bsz.a(3, "PRD", "No CDS is configured, enabling all variables");
        return bsx.a;
      }
      bsz.a(3, "PRD", "CDS field was found");
      localObject = new bsx(((String)localObject).trim());
      return (bsx)localObject;
    }
    catch (Exception localException)
    {
      bsz.a("PRD", "Failed to decode CDS", localException);
    }
    return bsx.a;
  }
}

/* Location:
 * Qualified Name:     bra
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */