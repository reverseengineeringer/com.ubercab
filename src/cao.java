import android.content.Context;
import android.view.View;
import android.webkit.WebView;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.ScrollView;
import android.widget.TextView;
import java.util.Arrays;
import java.util.HashSet;
import java.util.Locale;
import java.util.Set;

public final class cao
{
  public View a;
  public TextView b;
  public Button c;
  private ScrollView d;
  private LinearLayout e;
  private ImageView f;
  
  public cao(Context paramContext, can paramcan)
  {
    d = new ScrollView(paramContext);
    d.setBackgroundColor(but.d);
    e = new LinearLayout(paramContext);
    e.setOrientation(1);
    d.addView(e);
    buu.a(e, -1, -2);
    LinearLayout localLinearLayout = new LinearLayout(paramContext);
    localLinearLayout.setOrientation(1);
    buu.a(localLinearLayout, "16dip", "20dip", "16dip", "20dip");
    e.addView(localLinearLayout);
    buu.a(localLinearLayout, -1, -2);
    f = new ImageView(paramContext);
    f.setImageBitmap(buu.c("", paramContext));
    f.setContentDescription("PayPal");
    localLinearLayout.addView(f);
    buu.a(f, -1, "40dip");
    buu.b(f, null, null, null, "10dip");
    b = new TextView(paramContext);
    WebView localWebView = new WebView(paramContext);
    localWebView.setBackgroundColor(but.d);
    if (bvf.a) {}
    for (String str1 = "direction: rtl;";; str1 = "")
    {
      str1 = "body { color: #515151; line-height: 26px; font-size: 18px; font-family: sans-serif-light, DroidSans, sans-serif; font-weight: 300; " + str1 + "} h1, strong { font-family: DroidSans, sans-serif; font-weight: 500; }h1 { font-size: 18px }h1, p { margin: 0 0 13px 0 }ul { margin: 0; padding-left: 1.1em; list-style-position: outside; }";
      StringBuilder localStringBuilder = new StringBuilder("<html><meta name='viewport' content='width=device-width; initial-scale=1.0;'/><head><style type=\"text/css\">").append(str1).append("</style></head><body>");
      switch (cap.a[paramcan.ordinal()])
      {
      default: 
        paramcan = null;
        localWebView.loadData(paramcan + "</body><html>", "text/html; charset=UTF-8", null);
        localWebView.setClickable(true);
        localLinearLayout.addView(localWebView);
        buu.a(localWebView, -1, -2);
        buu.b(localWebView, null, null, null, "10dip");
        c = new Button(paramContext);
        localLinearLayout.addView(c);
        buu.a(c, true, paramContext);
        buu.b(c, null, "20dip", null, "4dip");
        a = d;
        return;
      }
    }
    String str2 = bvf.a(bvh.am);
    str1 = Locale.getDefault().getCountry().toLowerCase(Locale.US);
    paramcan = str1;
    if (bwy.c(str1)) {
      paramcan = "us";
    }
    if (new HashSet(Arrays.asList(new String[] { "nl", "de", "it", "fr", "gb", "us" })).contains(paramcan)) {}
    for (paramcan = String.format("https://www.paypal.com/%s/webapps/mpp/ua/useragreement-full", new Object[] { paramcan });; paramcan = "")
    {
      paramcan = String.format(str2, new Object[] { paramcan });
      break;
      paramcan = bvf.a(bvh.ai);
      break;
      paramcan = String.format(bvf.a(bvh.aH), new Object[] { bvf.a(bvh.aG) });
      break;
      paramcan = String.format(bvf.a(bvh.aH), new Object[] { bvf.a(bvh.aJ) });
      break;
    }
  }
}

/* Location:
 * Qualified Name:     cao
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */