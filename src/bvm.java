import com.paypal.android.sdk.g;
import java.util.Locale;
import org.json.JSONObject;

public final class bvm
{
  private String a;
  private String b;
  private String c;
  private Double d;
  private String e;
  private String f;
  
  private bvm(String paramString1, String paramString2, String paramString3, Double paramDouble, String paramString4, String paramString5)
  {
    a = paramString1;
    b = paramString2;
    c = paramString3;
    d = paramDouble;
    e = paramString4;
    f = paramString5;
  }
  
  public static bvm a(JSONObject paramJSONObject)
  {
    Object localObject3;
    Object localObject4;
    Object localObject2;
    Object localObject5;
    if (paramJSONObject.optJSONObject("bank_account") != null)
    {
      localObject1 = paramJSONObject.getJSONObject("bank_account");
      if (localObject1 != null)
      {
        localObject3 = ((JSONObject)localObject1).getString("bank_name");
        localObject4 = ((JSONObject)localObject1).getString("account_type") + " x-" + ((JSONObject)localObject1).getString("account_number");
        localObject1 = bvf.a(bvh.e);
        localObject2 = Double.valueOf(0.0D);
        localObject5 = paramJSONObject.optJSONObject("amount");
        if (localObject5 != null)
        {
          localObject2 = ((JSONObject)localObject5).getString("value");
          localObject1 = a((String)localObject2, ((JSONObject)localObject5).getString("currency"));
          localObject2 = Double.valueOf(Double.parseDouble((String)localObject2));
        }
        return new bvm((String)localObject3, (String)localObject4, (String)localObject1, (Double)localObject2, "", paramJSONObject.optString("funding_mode"));
      }
      return null;
    }
    Object localObject1 = paramJSONObject.optString("funding_instrument_type");
    if ((bwy.d((CharSequence)localObject1)) && (((String)localObject1).equals("BALANCE")))
    {
      localObject2 = paramJSONObject.optJSONObject("amount");
      paramJSONObject = bvf.a(bvh.e);
      localObject1 = Double.valueOf(0.0D);
      if (localObject2 != null)
      {
        localObject1 = ((JSONObject)localObject2).getString("value");
        paramJSONObject = a((String)localObject1, ((JSONObject)localObject2).getString("currency"));
        localObject1 = Double.valueOf(Double.parseDouble((String)localObject1));
      }
      return new bvm(bvf.a(bvh.ay), "", paramJSONObject, (Double)localObject1, "", null);
    }
    if (paramJSONObject.optJSONObject("payment_card") != null)
    {
      localObject3 = paramJSONObject.getJSONObject("payment_card");
      if (localObject3 != null)
      {
        localObject2 = ((JSONObject)localObject3).getString("type");
        if (bwy.c((CharSequence)localObject2))
        {
          localObject1 = bvf.a(bvh.bg);
          localObject2 = ((JSONObject)localObject3).getString("type");
          if (!bwy.d((CharSequence)localObject2)) {
            break label766;
          }
          if (!((String)localObject2).equals("AMERICANEXPRESS")) {
            break label706;
          }
          localObject2 = "";
        }
        for (;;)
        {
          localObject5 = "x-" + ((JSONObject)localObject3).getString("number");
          JSONObject localJSONObject = paramJSONObject.optJSONObject("amount");
          localObject3 = bvf.a(bvh.e);
          localObject4 = Double.valueOf(0.0D);
          if (localJSONObject != null)
          {
            localObject4 = localJSONObject.getString("value");
            localObject3 = a((String)localObject4, localJSONObject.getString("currency"));
            localObject4 = Double.valueOf(Double.parseDouble((String)localObject4));
          }
          return new bvm((String)localObject1, (String)localObject5, (String)localObject3, (Double)localObject4, (String)localObject2, paramJSONObject.optString("funding_mode"));
          if (((String)localObject2).equals("AMERICANEXPRESS"))
          {
            localObject1 = bvf.a(bvh.g);
            break;
          }
          if (((String)localObject2).equals("CARTAAURA"))
          {
            localObject1 = bvf.a(bvh.h);
            break;
          }
          if (((String)localObject2).equals("CARTEAURORE"))
          {
            localObject1 = bvf.a(bvh.i);
            break;
          }
          if (((String)localObject2).equals("CARTAPREPAGATAPAYPAL"))
          {
            localObject1 = bvf.a(bvh.g);
            break;
          }
          if (((String)localObject2).equals("CARTEBLEUE"))
          {
            localObject1 = bvf.a(bvh.j);
            break;
          }
          if (((String)localObject2).equals("COFINOGA"))
          {
            localObject1 = bvf.a(bvh.l);
            break;
          }
          if (((String)localObject2).equals("DELTA"))
          {
            localObject1 = bvf.a(bvh.m);
            break;
          }
          if (((String)localObject2).equals("DISCOVER"))
          {
            localObject1 = bvf.a(bvh.n);
            break;
          }
          if (((String)localObject2).equals("ELECTRON"))
          {
            localObject1 = bvf.a(bvh.o);
            break;
          }
          if (((String)localObject2).equals("JCB"))
          {
            localObject1 = bvf.a(bvh.p);
            break;
          }
          if (((String)localObject2).equals("MAESTRO"))
          {
            localObject1 = bvf.a(bvh.q);
            break;
          }
          if (((String)localObject2).equals("MASTERCARD"))
          {
            localObject1 = bvf.a(bvh.r);
            break;
          }
          if (((String)localObject2).equals("POSTEPAY"))
          {
            localObject1 = bvf.a(bvh.s);
            break;
          }
          if (((String)localObject2).equals("4ETOILES"))
          {
            localObject1 = bvf.a(bvh.t);
            break;
          }
          if (((String)localObject2).equals("TARJETAAURORA"))
          {
            localObject1 = bvf.a(bvh.u);
            break;
          }
          localObject1 = localObject2;
          if (!((String)localObject2).equals("VISA")) {
            break;
          }
          localObject1 = bvf.a(bvh.v);
          break;
          label706:
          if (((String)localObject2).equals("DISCOVER")) {
            localObject2 = "";
          } else if (((String)localObject2).equals("JCB")) {
            localObject2 = "";
          } else if (((String)localObject2).equals("MASTERCARD")) {
            localObject2 = "";
          } else if (((String)localObject2).equals("VISA")) {
            localObject2 = "";
          } else {
            label766:
            localObject2 = "";
          }
        }
      }
      return null;
    }
    if (paramJSONObject.optJSONObject("credit") != null)
    {
      if (paramJSONObject.optJSONObject("credit") != null)
      {
        localObject1 = bvf.a(bvh.e);
        localObject2 = Double.valueOf(0.0D);
        localObject3 = paramJSONObject.optJSONObject("amount");
        if (localObject3 != null)
        {
          localObject2 = ((JSONObject)localObject3).getString("value");
          localObject1 = a((String)localObject2, ((JSONObject)localObject3).getString("currency"));
          localObject2 = Double.valueOf(Double.parseDouble((String)localObject2));
        }
        paramJSONObject = paramJSONObject.optString("funding_mode");
        return new bvm(bvf.a(bvh.az), "", (String)localObject1, (Double)localObject2, "", paramJSONObject);
      }
      return null;
    }
    return null;
  }
  
  private static String a(String paramString1, String paramString2)
  {
    buv localbuv = buv.a();
    return btu.a(Locale.getDefault(), localbuv.c().a(), Double.parseDouble(paramString1), paramString2);
  }
  
  public final String a()
  {
    return a;
  }
  
  public final String b()
  {
    return b;
  }
  
  public final String c()
  {
    return c;
  }
  
  public final Double d()
  {
    return d;
  }
  
  public final String e()
  {
    return e;
  }
  
  public final String f()
  {
    return f;
  }
}

/* Location:
 * Qualified Name:     bvm
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */